...
struct board {
    ...
    uint32_t place = 0;
    std::unordered_map<int32_t, std::vector<std::pair<uint8_t, uint8_t>>> m{};
    uint8_t rows[5]{};
    uint8_t columns[5]{};
};
...
auto get_score(int32_t last, const board &b) {
    ...
    for (auto &&[k, v]: b.m) {
        if (k == last) continue;
        score += (int32_t) k * (int32_t) v.size();
    }
    return score * last;
}
...
auto calculate_min_max(const std::vector<int32_t> &nums, std::vector<board> boards) {
    for (uint32_t i = 0; i < nums.size(); i++) {
        for (auto &&b: boards) {
            ...
            for (auto &&pos: b.m.at(nums[i]))
                if (!--b.rows[pos.first] || !--b.columns[pos.second]) b.place = i;
            b.m.erase(nums[i]);
        }
    }
    ...
    while (true) {
        ...
        for (uint8_t col = 0; col < 5; ++col) {
            for (uint8_t row = 0; row < 5; ++row) {
                ...
                boards.rbegin()->m[num].push_back({row, col});
            }
        }
    }
}
